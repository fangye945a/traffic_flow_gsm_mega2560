...
#define UART_BUFFER_LENGTH 512    //Maximum length allowed for UART data
#define TOPIC_BUFFER_LENGTH 62    //Maximum length allowed Topic
#define MESSAGE_BUFFER_LENGTH  450 //Maximum length allowed data
...
class GSM_MQTT
{
  public:
    ...
    volatile char _ping_return_flag = 0;    //ping包回复标志
    volatile char GSM_ReplyFlag;

    char reply[10];
    volatile bool pingFlag = false;
    volatile char tcpATerrorcount = 0;
    volatile char Hard_Reset_count = 0;     //add by fangye 20170824
...
extern char tmp[128];         //消息缓存数组
extern char csq[4];          //信号强度
